...
func (s *service) connect(c context.Context, cr *rpc.ConnectRequest, dryRun bool) *rpc.ConnectInfo {
	...
	K8sConfig, err := worker_cluster.NewK8sConfig(cr.KubeFlags, s.env)
	if err != nil && !dryRun {
	...
	case s.sharedState.GetClusterNonBlocking() != nil && s.sharedState.GetClusterNonBlocking().K8sConfig.Equals(K8sConfig):
		if mns := cr.MappedNamespaces; len(mns) > 0 {
			...
			sort.Strings(mns)
			s.sharedState.GetClusterNonBlocking().SetMappedNamespaces(c, mns)
		}
		ingressInfo, err := s.sharedState.GetClusterNonBlocking().DetectIngressBehavior(c)
		if err != nil {
			return &rpc.ConnectInfo{
				Error:     rpc.ConnectInfo_CLUSTER_FAILED,
				ErrorText: err.Error(),
		...
		ret := &rpc.ConnectInfo{
			...
			ClusterContext: s.sharedState.GetClusterNonBlocking().K8sConfig.Context,
...
func (s *service) connectWorker(c context.Context, cr *rpc.ConnectRequest, k8sConfig *worker_cluster.K8sConfig) *rpc.ConnectInfo {
	...
	cluster, err := func() (*worker_cluster.K8sCluster, error) {
		c, cancel := client.GetConfig(c).Timeouts.TimeoutContext(c, client.TimeoutClusterConnect)
		defer cancel()
		cluster, err := worker_cluster.NewKCluster(c,
			k8sConfig,
			mappedNamespaces,
			worker_cluster.Callbacks{
	...
	dlog.Infof(c, "Connected to context %s (%s)", s.sharedState.GetClusterNonBlocking().Context, s.sharedState.GetClusterNonBlocking().Server)
	...
	s.scout <- func() ScoutReport {
		report := ScoutReport{
			...
			PersistentMetadata: map[string]interface{}{
				"cluster_id":        s.sharedState.GetClusterNonBlocking().GetClusterId(c),
	...
	if err = s.sharedState.GetClusterNonBlocking().WaitUntilReady(c); err != nil {
		s.cancel()
		return &rpc.ConnectInfo{
			Error:     rpc.ConnectInfo_CLUSTER_FAILED,
			ErrorText: err.Error(),
...
func run(c context.Context) error {
	...
	s.sharedState.LoginExecutor = auth.NewStandardLoginExecutor(env, &s.sharedState.UserNotifications, s.scout)
	...
	g.Go("server-grpc", func(c context.Context) (err error) {
		...
		svc := grpc.NewServer()
		rpc.RegisterConnectorServer(svc, worker_grpc.NewGRPCService(
			worker_grpc.Callbacks{
